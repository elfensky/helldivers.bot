...
Fetches a single season from the official Helldivers API (`get_snapshots`) and upserts it into the normalized `h1_*` tables plus the raw `rebroadcast_season` table. This is the **shared backfill pipeline** used by three callers:
...
4. Upserts into: `rebroadcast_season` (raw JSON), `h1_season`, `h1_introduction_order`, `h1_points_max`, `h1_snapshot`, `h1_event` (defend + attack)
