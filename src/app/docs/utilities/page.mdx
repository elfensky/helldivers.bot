...
4. `>= 10,000,000` → `(n / 1M).toFixed(1) + 'M'`
5. `>= 1,000` → `n.toLocaleString()` (with commas)
6. Otherwise → `String(n)`

**Examples:** `1_500_000_000` → `"1.5B"`, `25_000_000` → `"25.0M"`, `5_000_000` → `"5,000,000"`, `12345` → `"12,345"`, `847` → `"847"`.
...
**Used by:** `LastUpdated` (live hero-sidebar counter), `SystemOverview` (admin worker heartbeat display).

**Note:** when used inside a component that ticks on an interval, pass `now` as state so the React Compiler can't elide re-renders — the hidden `Date.now()` fallback looks like a pure function to static analysis. See `LastUpdated.jsx` for the pattern.
...
**Tests:** `src/__tests__/unit/shared/utils/game/getWarOutcome.test.mjs`
...
Evaluates how a live event is performing relative to the expected linear progress rate. Returns a pace object for active events, or `null` for completed/inactive events.

```ts
evaluateProgress(event: { start_time, end_time, points, points_max, status })
    → { status: 'ahead' | 'behind' | 'on_track', delta, deltaPercent, currentRate, requiredRate } | null
```

**Algorithm:**

1. Calculate expected points at current time assuming linear progress (`expectedRate × elapsedTime`).
2. Compare actual `points` against expected with a +10% upper buffer ('behind' has no buffer — any shortfall reports).
3. Return status:
    - `'ahead'` — actual > expected + 10% buffer
    - `'behind'` — actual < expected
    - `'on_track'` — in between
    - `null` — event `status` is not `'active'` or the time window is degenerate

The `'behind'` predicate is algebraically the complement of `eventForecast`'s `onTrack`, so the ▲/▼ pace indicator and the Behind/On track outcome verdict on event cards always agree (pinned by `paceVerdict.contract.test.mjs`).

**Tests:** `src/__tests__/unit/features/stats/evaluateProgress.test.mjs`
...
Discriminated union on the `action` field. Used by the `/api/h1/rebroadcast` endpoint after `Object.fromEntries(formData.entries())` converts the request body.
