...
The `'behind'` predicate is algebraically the complement of `eventForecast`'s `onTrack`, so the ▲/▼ pace indicator and the Falls/Fails outcome verdict on event cards always agree (pinned by `paceVerdict.contract.test.mjs`).
