...
export const metadata = {
    ...
    description:
        'How the defend-wave likelihood window was built: three modeling attempts, placebo-tested covariates, a reverse-engineered gamma scheduler, and the pre-registered gate that kept a countdown from shipping.',
...
Partially — and honestly. Defend waves ride one global, hidden timer, and after three modeling attempts we can put a calibrated likelihood window on the next one, but not a countdown. This page is the whole story: the rules of the game, two attempts that measured the wrong thing or hit a wall, the placebo discipline that killed four false discoveries, and a reverse-engineering of the scheduler itself.
...
| Cron-style tick grid | Combs in the lull at tick multiples; round wall-clock times | No comb at any tick tried (0.5h/1h/2h/3h/6h, all χ² ≈ df); 1,978 of 1,979 train starts carry non-zero seconds | Dead |
...
If we had to reconstruct the code, it would read:

```
onTrainEnd(result == WIN):
    nextWaveAt = now + sampleGamma(k ≈ 4.4, θ ≈ 8.9h)   // mean ≈ 39h, CV ≈ 0.48
    faction    = pickFaction()                          // drawn at spawn — see below
```

A KS distance of 0.073 does not prove the developers typed the word *gamma* — a delay built from a few exponential stages leaves the same fingerprint — but it rules out everything else on the list. This is the real lull histogram from the live database — drag k and watch the memoryless hypothesis miss:
...
Outside those regimes the draw is close to unpredictable. The transition matrix shows a mild same-again tilt — after a Cyborg wave the next one is Cyborgs 41.0% of the time against a 36.2% base rate, with Bugs at 33.4% vs 28.8% and Illuminate at 39.8% vs 35.0% — so "same faction again" is the best residual rule at 44.3% accuracy, and every other simple rule tried (most-overdue faction, highest liberation, majority class) lands between 35% and 42%. Nothing rule-like survives out there.
