...
3. We show the median with the 50% band around it and the within-24h/48h probabilities — the card reads like "~20h (14-32h) · 63% within 24h", the same median-first form as the assault ETA, never a bare countdown.
...
A fifth attempt (2026-08-01) asked the natural follow-up: since every piece of the assault epoch is now measurable — P(the assault fails | it's still running) climbs from 0.59 to 0.97 as the 48h timeout approaches; counterattack trains take a measurable ~7h at the median (they're *harder* to stop than normal trains: first-defend win rate 22.6% vs 46.7%); and a won assault **releases the paused clock immediately** (the next free wave lands at a median 0.0h after a successful assault ends — the gated timer had already fired and was being held) — can composing those pieces beat the statistical model for the *free* waves too? Answer: **no.** The composite posts the project's first sub-0.6 skill point estimate (0.588 [0.559, 0.621]) but fails calibration and sharpness, and on the honest paired comparison at assault moments it's flat against the KM cell (ratio 1.045, CI spanning 1). The blocker is the outcome fork itself: the fail and success futures sit ~60h apart, and P(fail)≈0.6 can't pick a side. The one signal that *can* — the same on-track/behind pace verdict the event cards ship — is nearly decisive in the data we have (an on-track assault past 35% elapsed has **never** failed; behind-pace ones fail ~70%) but rests on only **7 assaults** with progress history (S157+). That's a data clock, not a modeling problem: when ~30+ such assaults exist, fold the verdict into P(fail) and re-run. *(reproduce: `17-assault-outcome.mjs` · `18-outcome-composite.mjs`)*

**Correction (2026-08-03): the pace verdict was being measured against a deadline that never existed.** Script 14 scored the verdict by replaying it over `h1_event_progress` — but it used each event's *recorded* `end_time` as the deadline, and a won event's `end_time` is rewritten to the moment its points filled (rule 6). Every success was therefore judged against a deadline hours earlier than the one the live card sees, understating the time remaining and pushing the verdict toward "behind" on exactly the events that succeeded. Replaying against the nominal timer instead — the 150-minute defend clock, the 48h assault timeout — the verdict is far better than advertised: **91.6% accurate across 130 events and 2,193 moments**, rising from ~73% in the opening quarter to 92.4% past halfway and **99.7% in the final quarter**. Two consequences shipped. The anti-flicker margin, calibrated on the biased replay at 0.2, is now **0** — it was compensating for the bias, not for flicker, and removing it costs nothing on stability (flip p90 0.1, against 1.0 at every non-zero margin) while gaining five accuracy points. And the cards stay silent for the first **25%** of an event, where the since-start average rate is still noise. Script 17's verdict-conditioning table carried the same bias and was re-run with the 48h timeout restored: the attempt-5 reading above *strengthens* rather than moves — an on-track assault has now never failed in **any** elapsed bin (not merely past 35%), while behind-pace ones fail ~65% early and **100%** past 60% elapsed. It still rests on 8 assaults with progress history, so the data clock below is unchanged. *(reproduce: `14-event-verdict-margin.mjs` · `17-assault-outcome.mjs`)*
...
- **The free-wave band** (`waveForecast`) runs while the scheduler's dice are actually rolling — no assault, no active defend. It classifies the moment (SC9, SC10 or NORMAL), looks up the Kaplan-Meier remaining-wait table for that state at the current elapsed hour, and renders the median (the accent tick on the bar) with the 50% band around it in a subtler tone, plus the within-24h/48h probabilities — and two badges: **imminent**, when the within-24h probability crosses its threshold, and **running long**, whenever the state is SC9. During an assault it goes quiet on purpose: rule 8 says the invasion timer is paused, so a statistical band there would be dressing up a gate as a distribution.
- **The counteroffensive clock** (`counterattackForecast`) runs only while an assault is active — the regime the band just vacated. It is a mechanic, not a model: *"counterattack ⟨assault start + 48h⟩"* (earliest assault first when several run), phrased through the assault's own live pace verdict — *"assault behind pace · counterattack ⟨t⟩"* when the bar is not filling in time, *"assault on pace to succeed · counterattack ⟨t⟩ only if pace collapses"* when it is. The pace wording is the same ▲/▼ projection the event cards already show, deliberately never a probability (the verdict-to-outcome link waits on data — see the follow-ups below).

### The two forecasts, by the numbers

| | Counteroffensive clock (mechanical) | Free-wave band (statistical) |
| --- | --- | --- |
| Timing source | 48.0h timeout, exact in 544/544 fails; counterattack within 10 min of it (467/474 slot-free, p05–p95 = 0.0h) | Kaplan-Meier table over ~160 wars, by state × elapsed hour |
| Backtested error | Median **0.0h** vs the statistical model's 9.2h on the same 7,282 assault moments (win rate 89.4%) | Median miss **~8h** on a ~44h cycle; band calibrated within ±0.05 at every quartile |
| The catch | Conditional on the assault failing (~59% base rate; won assaults remove the faction instead) | Five modeling attempts, best skill CI [0.559, 0.621] against the ≤0.6 ship bar — never met, so band, not countdown |
...
| Piece | Origin | Status |
| --- | --- | --- |
| *When an assault will start* | Deterministic full-points trigger + the pace-window ETA (the shipped assault ETA line) | Passed the gate; ships |
| *How long a failing assault runs* | The exact 48h timeout | A constant, not a forecast |
| *Will it fail at all* | The on-track/behind completion verdict (`eventForecast`, the same ▲/▼ on the event cards) | Reused verbatim as the clock's conditional wording |
| *Chaining ETA → timeout from the SC9 window* | Tested (`16-counterattack-pipeline.mjs`) | **Rejected** — pace-forecast noise swamps the deterministic tail (ratio 1.296, CI spanning 1); the clock only speaks once the assault has actually started |

Its honesty rules are hard-coded: it says *likely*, never *will*; it renders a band, never a countdown; it hides entirely while a wave is active instead of inventing precision; and the typical ±8h miss is printed on the card itself. That is the deal defends offer — a calibrated window, not a clock. (With one exception earned above: *during a homeworld assault*, "if this assault fails, the counterattack lands the moment it times out" **is** a clock — a deterministic mechanic, not a model — and the card shows exactly that line, conditional phrasing included, whenever an assault is active.) Attacks offer a much better deal: a deterministic trigger and a forecast that cleared every leg of the same gate. <a href="/docs/predict/attack" data-umami-event="docs-predict-attack">That is the solved half of this report →</a>
