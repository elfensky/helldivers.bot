...
Outside those regimes the draw is close to unpredictable. The transition matrix shows a mild same-again tilt — after a Cyborg wave the next one is Cyborgs 41.0% of the time against a 36.2% base rate, with Bugs at 33.4% vs 28.8% and Illuminate at 39.8% vs 35.0% — so "same faction again" is the best residual rule at 44.3% accuracy, "majority among active-status factions" manages 42.4%, and the three other simple rules tried (most-overdue faction, highest liberation, majority class) all land at 35–36%. Nothing rule-like survives out there.
