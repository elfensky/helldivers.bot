...
export const metadata = {
    ...
    description:
        'Full report on Helldivers 1 event predictability across 160 seasons — attacks fire deterministically the moment a campaign reaches full points, defends run in trains with a measured but unshipped forecast.',
...
**This has not been measured yet.** No rate model has been built and no accuracy figure exists. What has changed is the shape of the problem: it is now a straightforward extrapolation to a known finish line, not a search for a hidden trigger. Two things bound how well it can work:

- **Pace is not steady.** Measured on the current season's 15-minute data, a faction's rate over the last hour can differ from its three-day average by an order of magnitude, and roughly one three-day window in five shows net *negative* progress.
- **Historical pace is coarse.** A rate estimated over a window shorter than a day is uncomputable for 156 of 160 seasons, so a short-window model cannot be backtested even though it would be the better live model. The honest configuration is one that history can validate and the live feed can only improve on.

<details>
<summary>In depth: the tests, and what the earlier analysis got wrong</summary>

`08-attack-trigger.mjs` runs three falsifiable tests. Each is designed so that a genuine soft threshold would produce a visibly different result:

1. **Staleness gradient.** A sensor artifact predicts liberation-at-attack climbing toward 100% as the reading gets fresher; a real soft threshold predicts a flat line. Measured: 94.28% → 100.00%.
2. **Fresh readings, listed individually.** No aggregation to hide behind — the table above.
3. **Trigger lag** on the 15-minute seasons — 1 to 14 minutes.

**What Phase 1 (`01-trigger-hunt.mjs`) actually found.** The original trigger hunt was directionally right and quantitatively understated. It reported:

| Variable | IQR ratio | p-value | Verdict then |
| --- | --- | --- | --- |
| liberation | 0.135 | 0.0005 | RULE-LIKE |
| sectorsCaptured | **0.000** | 0.0005 | RULE-LIKE |
| daysIntoSeason | 0.912 | 0.0515 | No rule — the control check |
| hoursSincePrevEventEnd | 0.847 | 0.0105 | No rule |
| playerPercentile | 1.004 | 0.6592 | No rule |

`sectorsCaptured` came back with an interquartile range of **exactly zero**. That is not a tight distribution; that is a constant, and it was the determinism showing through. It got filed as "rule-like" alongside a liberation figure whose spread was entirely sensor lag. The control check on `daysIntoSeason` remains valid and still rules out a season-calendar artifact.

**The caveat that pointed at this and drew the wrong conclusion.** The earlier version of this page noted that coarse campaign-state resolution "can only *weaken* a real signal, never manufacture one — which is why this attack-side finding is trusted." The first half is true. The second does not follow. Coarse sampling cannot manufacture a signal from noise, but it can convert a **hard rule into a plausible-looking distribution** — and that distribution then gets published as a finding, complete with quartiles. That is what happened here, and it is the more useful lesson than the one originally drawn.
