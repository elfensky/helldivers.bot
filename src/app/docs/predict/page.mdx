...
**The one conditioning fact that matters is visible on the map today:** if any faction has 9 of 10 sectors captured, the current lull is running long — the median wait in that state is ~55h against ~35h otherwise (+20h, placebo-tested, consistent across all four eras of the game's history). The mechanism is coherent with the attack rules above: 9 sectors is the homeworld-assault threshold, and defend trains go quiet while an assault is imminent or running.
...
| --- | --- | --- | --- | --- | --- |
| Baseline (featureless, from the table above) | PASS | 0.753 [0.732, 0.773] | 9.1h | 23.1h — not narrower | INCONCLUSIVE |
| State-conditional, plain (v1) | FAIL (all rates low) | 0.644 [0.624, 0.666] | 7.8h | 18.9h — **narrower** | INCONCLUSIVE |
| Season-pace adaptive | PASS | 0.787 [0.763, 0.813] | 9.5h | 24.2h — not narrower | INCONCLUSIVE |
| **State-conditional, KM-corrected (v2)** | **PASS** | **0.648 [0.622, 0.674]** | **7.8h** | **20.4h — narrower** | **INCONCLUSIVE** |

The KM configuration is the first model in this project to pass the sharpness leg, and it passes calibration — two of three gate legs, with a skill ratio a clear CI-separated step below every previous attempt. It dies on the leg the gate is strictest about: the skill-ratio CI's upper bound is 0.674 against the ≤ 0.6 ship requirement. Two caveats are disclosed with it: the winning configuration is the post-hoc best of six correlated variants (selection its CI doesn't account for — and it *still* misses), and `h1_status`'s ~daily staleness smears the state boundaries, so the true assault-window effect is likely sharper than measured — a real reason to revisit once enough high-resolution seasons (S157+, 15-minute buckets, only ~49 train starts so far) accumulate.
