...
**Revisited with faction telemetry (attempt 4).** The player idea got a second, fairer trial once four seasons of 15-minute per-faction player counts existed (S157–160): walk-forward, every variant scored on identical moments, paired against the shipped model. The crude ratio above reproduced as worse (+2.4h median paired error, pooled). The smarter forms — points per player<sup>α</sup>-hour integrated over a predicted player curve, and the day-of-week table replaced by the faction's own hour-of-week player pattern — were *directionally* promising at best: the best variant (α = 0.7) came out −1.0h pooled and won both Cyborg seasons, but with only 2–3 scorable attacks per faction that is a coin read, not a result. The shipped model stays; the experiment reruns unchanged as each new war adds a telemetry season.
