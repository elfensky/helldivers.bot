...
import { DEFINITION_LR, DEFINITION_TD } from './dataFlowDefinition';
...
A single normalized schema (`h1_season`, `h1_status`, `h1_statistic`, `h1_event`, `h1_event_progress`) is the source of truth. The `/api/h1/rebroadcast` endpoint reconstructs the official wire format on demand — there are no separate raw-cache tables.
