...
- **PWA:** Serwist (`@serwist/next`) generates service worker at build time with automatic precache manifest (content-hash based). `skipWaiting: true` for immediate updates. Custom push notification handlers in `src/sw.js`. `public/sw.js` is a gitignored build artifact. Last poll payload cached in localStorage for offline fallback.
