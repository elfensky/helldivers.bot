...
- **Data-source separation:** The two official HD1 endpoints feed disjoint table sets and **must not interact**. `get_campaign_status` → `h1_live` (current-war live stats consumed only by the homepage live section) + `h1_event` (new events as they happen); populated every poll by `updateStatus`. `get_snapshots` → `h1_snapshot` + `h1_event` (historical seasons, consumed only by `/archives`); populated by the worker during the active season and by `fetchAndSeedSeason` for on-demand backfill. `fetchAndSeedSeason` **must never touch `h1_live`** — past-season combat stats are intentionally absent from `/archives` for cold-backfilled seasons. `h1_live_snapshot` is currently write-only storage for future trend analytics; the only reader is `snapshotTimers.mjs` seeding its throttle bootstrap.
